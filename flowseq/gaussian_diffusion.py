...
class GaussianDiffusion:
    ...
    def training_losses_seq2seq(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        tT_loss = terms["tT_loss"] = mean_flat(out_mean**2)

        terms["nll"] = self._token_discrete_loss(
            x_start, get_logits, input_ids_x
        )  # embedding regularization
        terms["loss"] = terms["mse"] + terms["nll"] + terms["tT_loss"]

        terms["nll_est"] = self._token_discrete_loss(
            model_out_x_start,
            get_logits,
            input_ids_x,
            mask=input_ids_mask,

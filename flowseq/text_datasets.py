...
def load_data_text(
    ...
    training_data = get_corpus(
        data_args, seq_len, split=split, loaded_vocab=loaded_vocab, is_debug=is_debug
    )
...
def get_corpus(data_args, seq_len, split="train", loaded_vocab=None, is_debug=False):
    print(
        "#" * 30,
    ...
    if is_debug:
        sentence_lst["src"] = sentence_lst["src"][:11111]
        sentence_lst["trg"] = sentence_lst["trg"][:11111]
        print("### Debug mode, only use 11111 samples")

    print(f"data split {split} length: {len(sentence_lst['src'])}")
...
class TextDataset(Dataset):
    def __init__(self, text_datasets, data_args, model_emb=None):
        ...
        print("TextDataset length", self.length)
        print("TextDataset model_emb.shape", self.model_emb.weight.shape)
